...
def re_fetch_1_at_a_time_if_necessary(urls)
  ...
  urls.each do|url|
    ...
    if File.stat(filename).size > 193 #kludge valid jpeg files are 194 bytes or bigger
      $stderr.printf("skipping VALID JPEG filename:%s\n", filename)
      next
    end
    try_count = 0
    begin
...
re_fetch_1_at_a_time_if_necessary(urls)
